...
From this chart, it is very clear that the votes are dominated by a very small number of countries. I wanted to get a closer look at what countries had the most votes so I replotted with only countries representing the top quintile of voting countries.
...
top_director <- subset(votesbyDirector, Totals >= quantile(Totals,0.975))
#Create chart
ggplot(aes(x=reorder(selectionDirector,-Totals), y=Totals, color=Active_Decade, fill=Active_Decade), data=top_director) +
  geom_point(size = 4)+ 
